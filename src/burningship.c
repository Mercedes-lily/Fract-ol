...
/*   Updated: 2023/07/24 15:46:31 by vst-pier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	b_equation(t_set *fract)
{
		...
		&& fract->nb_iteration <= MAX_ITER)
	{
		...
		result_y = 2 * fabs(zn_x * zn_y) - fract->var_y;
		fract->nb_iteration++;
		zn_x = result_x;
		zn_y = result_y;
	}
...
void	my_scroll_hook_b(double xdelta, double ydelta, void	*param)
{
	...
	if (fract->color_name == 'b')
		pixel_to_image(fract, blue_pink, b_equation);
	else if (fract->color_name == 'p')
		pixel_to_image(fract, pink, b_equation);
	else if (fract->color_name == 'o')
		pixel_to_image(fract, orange, b_equation);
}
...
void	all_hooks_b(t_set *fract, char *str)
{
	...
	mlx_scroll_hook(fract->mlx, my_scroll_hook_b, fract);
}
...
int	burning_ship(void)
{
	...
	pixel_to_image(fract, orange, b_equation);
	all_hooks_b(fract, "Burning ship Set");
	mlx_loop(fract->mlx);
